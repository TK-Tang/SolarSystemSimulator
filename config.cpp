...
void Config::zoomOut(Zoom *alter) {
    alter->Zoom::zoomOut(Config::getInstance());
}

void Config::zoomIn(Zoom *alter){
    alter->Zoom::zoomIn(Config::getInstance());
}
/* TO IMPLEMENT:
 *

 * 3) Pan - Modify hard coordinates
 * 4) Change time setp
 * 5) Default centre
 * 6) info panel
 * 7) STRATEGY DESIGN
