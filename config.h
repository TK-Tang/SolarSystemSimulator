...
public:
    ...
    // set the distance scale for zooming in the window
    void setDistanceScale( double scale ){ m_distanceScale = scale;}

    // scale the radius of celestial objects
    void setRadiusScale( double scale ){ m_radiusScale = scale;}

    // scale the log point radius
    void setLogPointRadius( double scale ) { m_logPointRadius = scale;}

    //Calls the zoom out method in the Zoom class
    void zoomOut(Zoom* z);
    //Calls the zoom in method in the Zoom class
    void zoomIn(Zoom* z);

    //sets the physics step size
    void setPhysicsStepSize(int step){
        m_physicsStepSize = step;
    }
